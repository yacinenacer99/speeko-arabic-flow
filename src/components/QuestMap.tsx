...
const stages: Stage[] = [
  ...
  { name: "مؤثر", desc: "رتّب أفكارك", criteria: "٥ جلسات بنقاط تدفق أكثر من ٧٥", status: "locked" },
  { name: "خطيب", desc: "أي موضوع، أي غرفة", criteria: "٥ تحديات مفاجئة بنتيجة أكثر من ٧٠", status: "pro-locked" },
  { name: "مؤثر متقدم", desc: "تحت الضغط", criteria: "١٠ تحديات متقدمة بنتيجة أكثر من ٨٠", status: "pro-locked" },
            ...
            <div key={i} className="relative">
              ...
              >
                ...
                <div className="hidden md:flex w-full" style={{ justifyContent: isLeft ? "flex-start" : "flex-end" }}>
                  ...
                  {/* Right card (when !isLeft on desktop) */}
                  {!isLeft && (
                    <>
                      <div className="flex flex-col items-center" style={{ position: "relative" }}>
                        <div style={{ width: 24, height: 2, background: spineColor, marginTop: 23 }} />
...
const SpineNode = ({ status }: { status: Stage["status"] }) => {
  if (status === "completed") {
    return (
      <div
        style={{
...
const StageCard = ({ stage, side }: { stage: Stage; side: "left" | "right" }) => {
  const isCompleted = stage.status === "completed";
  const isCurrent = stage.status === "current";
  const isLocked = stage.status === "locked" || stage.status === "pro-locked";
  const isPro = stage.status === "pro-locked";
